...
def transfer_imgs_from_path(im_path, temp_dir):
    ...
    if os.listdir(im_path):  # if there are any files in the user input folder
        for i in os.listdir(im_path):
            ...
            if file_extension in {'.exr', '.png', '.jpg', '.jpeg', '.EXR', '.PNG', '.JPG', '.JPEG'}:
                imgs.append(i)
    else:
...
def copy_matching_files(input_path, cache_dir):
    ...
    if "#" not in input_path:
        raise ValueError("The input path must contain '#' as a placeholder for frame numbers.")
    ...
    for file_name in os.listdir(base_dir):
        if file_name.startswith(base_name) and file_name.endswith(os.path.splitext(input_path)[1]):
