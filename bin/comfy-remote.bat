@echo off
setlocal
REM comfy-remote.bat — always exposes repo/src on PYTHONPATH and runs the pure launcher.

REM 1) Locate repo root and src/
set "SCRIPT_DIR=%~dp0"
for %%I in ("%SCRIPT_DIR%..") do set "ROOT=%%~fI"
set "SRC_DIR=%ROOT%\src"
if exist "%SRC_DIR%\comfyui_remote\bin\comfy_server.py" (
...
REM 2) If you want to force a specific interpreter, set COMFY_REMOTE_PY to python.exe
if defined COMFY_REMOTE_PY (
  if exist "%COMFY_REMOTE_PY%" (
    "%COMFY_REMOTE_PY%" -m comfyui_remote.bin.comfy_server %*
    exit /b %ERRORLEVEL%
  ) else (
...
where py >nul 2>&1
if %ERRORLEVEL%==0 (
  py -3 -m comfyui_remote.bin.comfy_server %*
  exit /b %ERRORLEVEL%
)

where python >nul 2>&1
if %ERRORLEVEL%==0 (
  python -m comfyui_remote.bin.comfy_server %*
  exit /b %ERRORLEVEL%
)
